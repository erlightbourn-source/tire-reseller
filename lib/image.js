...
const DATA_URI_RE = /^data:(image\/(?:png|jpe?g|gif|webp));base64,([A-Za-z0-9+/=]+)$/;
const DATA_URI_EXT = { "image/png": "png", "image/jpeg": "jpg", "image/jpg": "jpg", "image/webp": "webp", "image/gif": "gif" };

/**
 * The upload pipeline (`app/api/upload`) strips Exif/GPS before a photo ever
 * gets a URL — but `lib/validation.js#isAllowedPhotoUrl` also accepts raw
 * `data:image/...;base64,...` photo values submitted directly to the listing
 * create/edit APIs, bypassing that pipeline entirely. This mirrors
 * `stripMetadata` for that path so a data-URI photo can't ship a seller's GPS
 * location. Same conservative contract: anything that doesn't parse as a
 * data URI, or whose payload doesn't decode as one of these image types, is
 * returned unchanged rather than risk corrupting it.
 */
export function stripDataUriMetadata(uri) {
  const m = typeof uri === "string" ? DATA_URI_RE.exec(uri) : null;
  if (!m) return uri;
  const mime = m[1] === "image/jpg" ? "image/jpeg" : m[1];
  const ext = DATA_URI_EXT[mime];
  let buf;
  try {
  ...
  const stripped = stripMetadata(buf, ext);
  return `data:${mime};base64,${stripped.toString("base64")}`;
}
