 ...
 * Scans EVERY comma segment (not just the last), so it survives the common
 * Google-Places format "539 Charles St, Providence, RI 02904, United States"
 * where the state sits mid-string and the tail is "United States". Also
 * tokenizes each segment so "RI 02904" (state + zip together) still resolves.
 */
export function stateFromLocation(loc) {
  ...
  for (const seg of segs) {
    ...
    if (BY_ABBR[up]) return up; // exact abbr segment, e.g. "FL"
    const byName = STATES.find((s) => s.name.toLowerCase() === seg.toLowerCase());
    if (byName) return byName.abbr; // full state name segment, e.g. "Rhode Island"
    for (const tok of up.split(/\s+/)) {
      if (BY_ABBR[tok]) return tok; // abbr inside a segment, e.g. "RI" in "RI 02904"
    }
  }
  // Whole string is a bare state name, e.g. "Florida"
  const whole = STATES.find((s) => s.name.toLowerCase() === text.toLowerCase());
  return whole ? whole.abbr : null;
}
