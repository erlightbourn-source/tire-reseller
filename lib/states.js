...
/**
 * Resolve a listing's state: an explicit 2-letter selection wins (e.g. from the
 * new-listing form's required State dropdown), else fall back to parsing the
 * free-text location. Guarantees a state whenever the seller picked one, so a
 * bare-city location like "Coral Springs" no longer saves state = null.
 */
export function resolveState(explicit, location) {
  if (isStateAbbr(explicit)) return String(explicit).toUpperCase();
  return stateFromLocation(location);
}

/** A user's home state: their saved profile state, else parsed from location. */
export function userStateOf(user) {
  if (!user) return null;
  if (user.state && isStateAbbr(user.state)) return user.state.toUpperCase();
  return stateFromLocation(user.location);
}

/**
 * Extract a 2-letter state code from a free-text location.
 *
 * Two-pass so a mid-string state resolves without a loose token scan
 * mis-firing on city names that contain a state abbreviation:
 *   Pass 1 — an EXACT whole-segment match (abbr or full name) across all
 *            comma segments. Handles the common "City, ST" and any position,
 *            e.g. "La Mesa, CA" → CA (never "LA" from the city token).
 *   Pass 2 — only on a miss, a ZIP-GUARDED token scan: a 2-letter abbr
 *            immediately followed by a 5-digit ZIP, e.g. "RI 02904" → RI
 *            (survives the Google-Places tail "…, United States"), while
 *            "La Mesa" never matches because "LA" isn't ZIP-followed.
 */
export function stateFromLocation(loc) {
  ...
  // Pass 1: exact whole-segment abbr or full state name.
  for (const seg of segs) {
    const up = seg.toUpperCase();
    if (BY_ABBR[up]) return up; // e.g. "FL", "CA"
    const byName = STATES.find((s) => s.name.toLowerCase() === seg.toLowerCase());
    if (byName) return byName.abbr; // e.g. "Rhode Island", "Florida"
  }

  // Pass 2: ZIP-guarded token scan — abbr directly before a 5-digit ZIP.
  for (const seg of segs) {
    const toks = seg.toUpperCase().split(/\s+/);
    for (let i = 0; i < toks.length - 1; i++) {
      if (BY_ABBR[toks[i]] && /^\d{5}(-\d{4})?$/.test(toks[i + 1])) {
        return toks[i]; // e.g. "FL 33065", "RI 02904"
      }
    }
  }
