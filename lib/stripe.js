...
function keyConfigured() {
  ...
  // Accept both standard secret keys (sk_) and restricted keys (rk_). A
  // restricted key is a full, valid credential for the Stripe SDK (checkout,
  // customers, webhooks) with a narrower scope — prod uses rk_live_.
  return (key.startsWith("sk_") || key.startsWith("rk_")) && !key.includes("REPLACE_ME");
}
