...
// bcrypt silently ignores bytes past 72 (and chokes on NUL bytes). Pre-hashing
// with SHA-256 → base64 feeds bcrypt a fixed 44-byte input, so the full password
// always contributes and there's no truncation ceiling. Cost 12 (~250ms) raises
// the offline-cracking bar; logins are rate-limited so the cost is fine.
 ...
 *   "free"    — seller inside their launch listing window (free, no card;
 *               `sellerFreeUntil`) — pricing itself lives in lib/pricing.js
 *   "paid"    — seller with an active paid subscription
 *   "expired" — seller whose launch window ended and isn't paying yet
 */
...
/**
 * End of a new seller's launch listing window (free, no card), as a Date.
 * Length is a product decision (currently 12 months from signup); the copy
 * never promises "a year" — it says "free during launch". Prices: lib/pricing.js.
 */
