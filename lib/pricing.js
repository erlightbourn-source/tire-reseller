...
//
// TireKind has ONE seller plan. Listing is free during launch (no card; the
...
export const PLAN_NAME = "TireKind Seller";
 ...
 * At zero it reads as an open offer ("All 25 founding spots open"): a public "0 of 25 claimed"
 * tells a first-time seller that nobody has joined yet (Marcus design audit 2026-09-24).
 */
export function foundingSpotsLine(claimed) {
  if (!Number.isInteger(claimed) || claimed < 0) return null;
  if (claimed === 0) return `All ${FOUNDING_SEATS} founding spots open`;
