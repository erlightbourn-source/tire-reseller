...
// Our Vercel Blob store's public host. Explicit env first, else derived from the
// read-write token (vercel_blob_rw_<storeId>_...), same rule as lib/tiktok.js.
// null = no blob store configured, so no blob URL can be ours.
export function ownBlobHost() {
...
function isOwnBlobUrl(u) {
  ...
  const m = /^https:\/\/([a-z0-9-]+\.public\.blob\.vercel-storage\.com)\//.exec(u);
  return !!host && !!m && m[1] === host;
}
