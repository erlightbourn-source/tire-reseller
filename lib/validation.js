...
// unit-testable in a plain Node process. (lib/image.js is likewise pure Buffer
// code with no next/server import; a RELATIVE import — `@/*` aliases don't
// resolve under plain `node --test`, only inside the Next.js build.)
...
// Only inline `data:` photos skip /api/upload's EXIF/GPS stripping — a
// listing can be created/edited with photos already embedded as base64 data
// URIs in the request body, never touching storeLocal/storeR2's stripMetadata
// call. Re-strip here so every photo path is GPS-safe, not just the upload
// widget's. Host-served (/uploads/, R2, blob) URLs pass through untouched —
// their bytes were already stripped at upload time.
const DATA_IMG_PARSE_RE = /^data:image\/(png|jpe?g|gif|webp);base64,([A-Za-z0-9+/=]+)$/;
const DATA_IMG_EXT = { png: "png", jpeg: "jpg", jpg: "jpg", gif: "gif", webp: "webp" };

export function sanitizePhotoUrl(u) {
  const m = typeof u === "string" && u.match(DATA_IMG_PARSE_RE);
  if (!m) return u; // not a data URI (or already rejected by isAllowedPhotoUrl) — leave as-is
  const [, mime, b64] = m;
  const ext = DATA_IMG_EXT[mime.toLowerCase()];
  try {
    const stripped = stripMetadata(Buffer.from(b64, "base64"), ext);
    return `data:image/${mime};base64,${stripped.toString("base64")}`;
  } catch {
    return u; // never corrupt/drop a photo over a stripping failure
  }
}
